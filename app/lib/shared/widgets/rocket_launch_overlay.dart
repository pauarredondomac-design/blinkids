...
class _RocketLaunchScreenState extends State<_RocketLaunchScreen>
  ...
  void initState() {
    ...
    _video.initialize().then((_) {
      ...
      setState(() {
        _videoReady = true;
        _textCtrl = AnimationController(vsync: this, duration: _video.value.duration)
          ..forward();
      });
      _video.play();
    });
  ...
  void _onVideoTick() {
    ...
    if (v.isInitialized && v.duration > Duration.zero &&
        !v.isPlaying && v.position >= v.duration && !_showUnlocked) {
...
class _ForestUnlockedCard extends StatelessWidget {
  const _ForestUnlockedCard({required this.onGoToForest, required this.onDismiss});
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: const Color(0xFF69F0AE).withOpacity(0.6), width: 1.6),
              boxShadow: [BoxShadow(color: const Color(0xFF69F0AE).withOpacity(0.25), blurRadius: 24, spreadRadius: 2)],
            ),
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                ...
                ClipRRect(
                  ...
                  child: forest.unlockedImagePath != null
                      ? Image.asset(forest.unlockedImagePath!, width: 220, height: 140, fit: BoxFit.cover)
                      : Container(width: 220, height: 140, color: forest.accentColor.withOpacity(0.3),
                          child: const Center(child: Text('🌲', style: TextStyle(fontSize: 56)))),
                ),
                const SizedBox(height: 14),
                Text('¡Desbloqueaste el ${forest.name}! 🌲',
                    textAlign: TextAlign.center,
                    style: const TextStyle(fontFamily: 'Nunito', fontWeight: FontWeight.w800, fontSize: 16, color: Color(0xFF69F0AE))),
                ...
                SizedBox(
                  ...
                  child: FilledButton(
                    ...
                    child: const Text('Ir al Bosque 🌲', style: TextStyle(fontFamily: 'Nunito', fontWeight: FontWeight.w800, fontSize: 15)),
                  ),
                ),
                const SizedBox(height: 8),
                TextButton(
                  onPressed: onDismiss,
                  child: const Text('Seguir explorando', style: TextStyle(fontFamily: 'Nunito', color: Colors.white54)),
                ),
              ],
            ),
          ),
